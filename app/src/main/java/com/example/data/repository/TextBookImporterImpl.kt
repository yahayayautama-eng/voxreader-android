...
class TextBookImporterImpl @Inject constructor(
     ...
     */
    override suspend fun invoke(bookId: String): RedetectResult = withContext(Dispatchers.IO) {
        try {
            val path = database.bookDao().getSourceFilePath(bookId)
                ?: return@withContext RedetectResult.Error("Original file isn't available for this book.")
            val file = File(path)
            if (!file.exists()) {
                return@withContext RedetectResult.Error("Original file is no longer on this device.")
            }
            val filename = file.name
            val importedBook = parseByExtension(file, filename)
            ...
            if (chunks.isEmpty()) {
                return@withContext RedetectResult.Error("No readable text found in the original file.")
            }
            ...
            database.withTransaction {
                ...
                database.bookDao().updateTotalChapters(bookId, sections.size)
            ...
            RedetectResult.Error("Could not re-scan this document.")
        }
    }
    ...
    override fun invoke(uri: Uri): Flow<ImportState> = flow {
        ...
        try {
            ...
            val importedBook = parseByExtension(destinationFile, displayName)
            ...
            destination?.delete()
            emit(ImportState.Error("File is too large. Maximum supported size is 100MB."))
        } catch (exception: EpubParser.ParseException) {
            destination?.delete()
            emit(ImportState.Error(exception.message ?: "Could not read this EPUB file."))
            ...
            destination?.delete()
            emit(ImportState.Error(exception.message ?: "Could not read this DOCX file."))
    ...
    override fun invoke(pages: List<Bitmap>, title: String): Flow<ImportState> = flow {
        emit(ImportState.Importing(0f))
        try {
            if (pages.isEmpty()) {
                emit(ImportState.Error("No pages captured."))
                return@flow
            }
            val sections = pages.mapIndexed { index, bitmap ->
                emit(ImportState.Importing(0.1f + 0.6f * (index.toFloat() / pages.size)))
                val text = try {
                    ocrScanner.recognize(bitmap)
                } catch (exception: CancellationException) {
                    throw exception
                } catch (_: Exception) {
                    ""
                }
                ImportedSection("Page ${index + 1}", text.trim())
            }
            val importedBook = ImportedBook(
                ...
                sections = sections,
                coverBitmap = pages.first()
            )
            emit(ImportState.Importing(0.8f))
            emit(persistBook(importedBook, sourceFilePath = null, "No readable text found in the scanned pages."))
        } catch (exception: CancellationException) {
            throw exception
        } catch (_: Exception) {
            emit(ImportState.Error("Could not process the scanned pages."))
        }
    }.flowOn(Dispatchers.IO)
    ...
    private fun parseText(file: File, filename: String): ImportedBook {
        ...
        val sections = ChapterDetector.split(content)?.map { ImportedSection(it.title, it.content) }
            // No headings: group paragraphs into readable stretches rather than one section each.
     ...
     */
    private fun parseByExtension(file: File, displayName: String): ImportedBook {
        ...
        return when {
            ...
            stored.endsWith(".pdf", ignoreCase = true) -> parsePdf(file, displayName)
    ...
    private fun parsePdf(file: File, filename: String): ImportedBook {
        val pdf = PdfBookParser.parse(context, file)
        val fullText = pdf.pages.filter { it.isNotBlank() }.joinToString("\n\n")
        // Prefer the PDF's own bookmarks — an authored table of contents beats any guess. Failing that,
        // detect headings, then group into readable stretches. One section per page is the last resort:
        // a page break is a print artifact, not a chapter boundary.
        val structured = PdfBookParser.sectionsFromOutline(pdf.pages, pdf.outline)
        ...
        return ImportedBook(
            title = pdf.title.takeUnless { it.isNullOrBlank() }
                ?: filename.removeSuffix(".pdf").ifBlank { "Imported PDF" },
            author = pdf.author.takeUnless { it.isNullOrBlank() } ?: "Unknown Author",
            description = "Imported text-based PDF document",
            sections = grouped ?: pdf.pages.mapIndexedNotNull { index, pageText ->
    ...
    private data class ImportedSection(val title: String, val content: String)

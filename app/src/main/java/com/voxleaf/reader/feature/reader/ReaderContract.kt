...
enum class ReaderTheme {
    LIGHT, DARK, SEPIA, NIGHT, OLED, IVORY
}
...
sealed interface ReaderUiAction {
    ...
    data class OnChangeFontFamily(val fontFamily: ReaderFontFamily) : ReaderUiAction
    data class OnChangeLineSpacing(val multiplier: Float) : ReaderUiAction
    data class OnChangeFontSize(val deltaSp: Int) : ReaderUiAction
    data class OnSetTtsSpeed(val rate: Float) : ReaderUiAction
    data class OnSetTtsVoice(val voiceId: String) : ReaderUiAction
    data class OnSetTtsEngine(val engineId: EngineId) : ReaderUiAction
